import{S as e}from"./index-D9kWJpzA.js";import"./mainUVVaryingDeclaration-7dlJXaru.js";import"./oitFragment-RhDIA9a1.js";const t="pbrFragmentExtraDeclaration",E=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=E);const i="subSurfaceScatteringFunctions",h=`bool testLightingForSSS(float diffusionProfile)
{return diffusionProfile<1.;}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=h);const o="importanceSampling",L=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[o]||(e.IncludesShadersStore[o]=L);const a="pbrHelperFunctions",D=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[a]||(e.IncludesShadersStore[a]=D);const r="pbrDirectLightingSetupFunctions",p=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=p);const n="pbrDirectLightingFalloffFunctions",v=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[n]||(e.IncludesShadersStore[n]=v);const l="hdrFilteringFunctions",A=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[l]||(e.IncludesShadersStore[l]=A);const f="pbrDirectLightingFunctions",m=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[f]||(e.IncludesShadersStore[f]=m);const c="pbrIBLFunctions",S=`#if defined(REFLECTION) || defined(SS_REFRACTION)
...
#endif
`;e.IncludesShadersStore[c]||(e.IncludesShadersStore[c]=S);const s="pbrBlockNormalGeometric",N=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=N);const d="pbrBlockImageProcessing",C=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[d]||(e.IncludesShadersStore[d]=C);const g="pbrBlockPrePass",_=`#if SCENE_MRT_COUNT>0
...
#endif
#endif
`;e.IncludesShadersStore[g]||(e.IncludesShadersStore[g]=_);const u="pbrDebug",I=`#if DEBUGMODE>0
